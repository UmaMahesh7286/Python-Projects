...
from .service import BookingHistoryService
...
@api_view(['GET'])
def getSeatsByScheduleId(request, schedule_id):
    seats = Seats.objects.filter(schedule=schedule_id)
    serializer = SeatSerializer(seats, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)
        ...
        try:
            with trance.atomic():
                ...
                if serializer.is_valid():
                    ...
                    retriveSerializer = BookingRetrieveSerializer(
                        instance=bookings)
                    ...
                    cursor.execute(
                        f"select sum(no_of_tickets_booked) from `booking-module`.booking_history where schedule_id={booking.scheduleId};")
                    ...
                    logger.info(
                        "sum of tickets {}".format(sum_of_tickets))
                    logger.info(
                        (booking.journeyDate - booking.bookingDate).days)

                    # getting schedule object for getiing flight seating capacity
                    response = reqs.get(
                        f"http://localhost:9003/schedule/getById/{booking.scheduleId}")
                    schedule_data = response.json()

                    seating_capacity = schedule_data['flight']['seating_capacity']
                    bookinghistory = BookingHistory(
                        available_tickets=seating_capacity -sum_of_tickets-len(passengers),
                    ...
                    available_seats = int(
                        seating_capacity-sum_of_tickets-len(passengers))

                    url = "http://localhost:9003/schedule/patchAvailableTickets/{}/{}/".format(
                        schedule_data['id'], available_seats)
...
def getBooking(request, id):
    if request.method == 'PUT':
        ...
        bh = BookingHistoryService()
        bh.deleteBh(booking.scheduleId, len(Passenger))
